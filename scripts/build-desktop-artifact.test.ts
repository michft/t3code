  ...
  it("switches desktop packaging product names to nightly for nightly builds", () => {
    ...
    assert.equal(
      resolveDesktopProductName("0.0.17-nightly.20260413.42", {
        T3CODE_DESKTOP_PRODUCT_NAME: "T3 Code (JJ Nightly)",
      }),
      "T3 Code (JJ Nightly)",
    );
  });

  it("isolates configured fork packaging identity", () => {
    const env = {
      T3CODE_DESKTOP_APP_ID: "com.michft.t3code.jj-nightly",
      T3CODE_DESKTOP_ARTIFACT_PREFIX: "T3-Code-JJ",
    };
    assert.equal(resolveDesktopAppId(env), "com.michft.t3code.jj-nightly");
    assert.equal(resolveDesktopArtifactPrefix(env), "T3-Code-JJ");
  });
